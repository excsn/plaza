...
The wire is defined once, in Rust, and mirrored where clients live: the Dart/Flutter mirror is kept honest by golden fixtures generated from the Rust crate's own tests, each vector emitted twice (msgpack as a client receives it, JSON as a human reads it), so the mirror is checked against the crate rather than against someone's reading of it. Browser JS needs no mirror at all: kind byte plus JSON text frame is `JSON.parse` territory, which is a deliberate property, not luck.
