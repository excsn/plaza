...
**A derived world buys a check a speed budget cannot make.** gow_3d's ground is a rule both ends compute from one seed rather than a payload anyone sends, which was done to save bytes and turned out to be worth more than that: the server can refuse a claim more than a jump's apex above the ground. A speed budget never sees a client hovering, because hovering costs no horizontal distance at all, so the second rule catches a whole class the first is blind to. Anything the two ends derive rather than exchange is available to a validator for free, and that is a reason to derive it beyond the bandwidth.

**Price the movement a caller actually makes, not the whole vector.** The same budget originally spent 3D distance, so a player walking uphill was charged for the climb and refused for going up a slope. A run speed is a speed over the ground; what a claim does vertically is the height rule's business. The general form is that a budget must measure the quantity it is a budget *for*, and one that quietly includes a second quantity refuses honest callers on exactly the terrain the feature was built for.

**A move the server initiates needs its own idempotent channel.** Under client authority nearly every position travels upward, and a respawn is the exception: the server puts the character on fresh footing and the client, which owns where it stands, has no way to derive that. Told nothing, it stays where it died and every claim it sends afterwards is outside the budget from its new position, so it is refused for the rest of the session. The shape that works is a **counter rather than a flag**: the move must be applied exactly once, and a flag that stays set fights every step afterwards while one that clears itself is lost with a dropped frame. The client compares the counter against the last it acted on, so a missed frame is caught by the next. That is the same reasoning as an idempotency key on a request that must not be replayed, and it arrives here for the same reason.
