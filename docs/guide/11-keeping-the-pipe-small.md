...
[spacemo](../../examples/spacemo/) is the one caution about the block itself, and it is worth reading before you trust a grid in three dimensions. `SpatialGrid` indexes two axes, which is right for a yard or an arena and right for most 3D games, since a landscape is locally 2.5D and a flat grid plus a height check is what shipping MMOs use. In open volume it stops being free, and the failure is the opposite of the obvious guess: a grid on `(x, z)` returns the **disc**, a superset of the sphere, so nobody is ever missed and a great deal is over-sent. Measured at **7.1x the bandwidth**, per client, while the game looks entirely correct. A height filter on what the query returned is exact at identical query cost, which is the fix; a genuinely volumetric grid sends exactly what the filter sends and has to justify itself on query cost alone. Its control run is the other half: at slab thickness the flat grid costs 1.00x.

It also names the question to ask before reaching for any of this chapter's blocks: **which of what you send could the receiver have worked out?** spacemo carries two projectiles that differ by one field, a straight shot and a homing one, and streaming both costs 17.3x what sending the straight one's spawn alone does. Compression makes a path cheaper; not sending a path the receiver could derive makes it free. The catch is that it only works where nothing can change the answer after the packet leaves, which is exactly what the homing half is there to mark.

That example also names the cost this chapter's blocks are least likely to catch. Priority, rest and delta all optimise **freshness** of a standing world. Entities that come and go pay at entry and exit instead, and in a firefight they dominate: 31.4 bolts at 410 bytes a frame against 7.8 ships at 116, so the transient half was 78% of the packet while the standing half sat still.
