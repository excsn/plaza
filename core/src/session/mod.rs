...
pub struct TargetedOp<Op, ID: AgentId> {
  /// Who caused these ops. `Agent::system()` for anything no client asked for.
  ///
  /// **Server-side only.** It is not sent: a frame is the kind byte and the
  /// ops. Keep setting it, because logs and your own logic read it, but if a
  /// client needs to know who did something, put that in the op, where you can
  /// also choose a seat index over a 64-bit identity.
