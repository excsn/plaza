...
**To add a transport:** write a socket pump that calls `ConnectionManager::{register, forward_incoming, deregister}` and delegates the `Session` trait to a `TransportSession`. `tcp.rs` is the smaller of the two to copy.
...
| The message envelope: `SessionMessage`, `Agent`, `AgentId`, `SnapshotData`. Here rather than in core because a wasm client cannot depend on core | [wire/src/envelope.rs](wire/src/envelope.rs) |
