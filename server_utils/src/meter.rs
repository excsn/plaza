...
impl RateMeter {
  ...
  /// Sets how long this has been accruing over. Idempotent, so it can be called
  /// every tick with the simulation clock.
  ...
  /// The total per second of elapsed time. Zero before any time has passed,
  /// rather than a division by zero or an infinity that renders as `inf` in a
  /// readout.
  ...
  /// The mean sample. Zero before anything has been recorded.
