...
///
/// Pin the committed copy from the server's own tests, so drift fails CI even
/// when nothing rebuilds:
///
/// ```ignore
/// let dart = std::fs::read_to_string("../../flutter/my_client/lib/wire_protocol.dart")?;
/// assert!(dart.contains(&format!("= {};", PROTOCOL)));
/// ```
pub fn emit_dart<P: AsRef<Path>>(sources: &[P], dart_path: impl AsRef<Path>) {
  ...
  let generated = dart_version_file(version_of(sources));
