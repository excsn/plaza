//! Standing the town up behind a WebSocket, and serving the browser client
//! from the same port.
...
use crate::logic::PoketoLogic;
use crate::protocol::{PlayerId, PoketoOp, PROTOCOL, TICK_HZ};
use crate::state::PoketoState;

type PoketoSession = ActixWsPlazaSession<PoketoOp, PlayerId, MsgPackCodec>;

const WASM_FILE: &str = "poketo.wasm";
...
async fn ws_route(
  req: HttpRequest,
  stream: web::Payload,
  session: web::Data<Arc<PoketoSession>>,
...
pub async fn serve(bind: &str, static_dir: Option<String>) -> std::io::Result<()> {
  ...
  let logic = PoketoLogic::new().with_clock(sim_clock);
  // No snapshot provider, and here that is not a compromise: a tile world's
  // steady state is already a complete description of what is in view, so a
  // joiner's first ordinary frame is everything a dump would have contained.
  ...
  tracing::info!(bind, "poketo listening (WebSocket at /ws)");
