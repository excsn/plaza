...
cp "$CARGO_TARGET_DIR/wasm32-unknown-unknown/release/poketo.wasm" "$here/static/poketo.wasm"
...
python3 "$root/../ws_client/check_js_imports.py" "$here/static/poketo.wasm" "$root/../ws_client/js/plaza_ws.js"

# 4. Shrink it if binaryen is available (optional; skipped silently otherwise).
if command -v wasm-opt >/dev/null 2>&1; then
  echo "==> optimising with wasm-opt"
  wasm-opt -Oz "$here/static/poketo.wasm" -o "$here/static/poketo.wasm"
fi
