...
use crate::sim::server::WORLD_SEED;
...
const WASM_FILE: &str = "seed_defense.wasm";
...
pub async fn serve(bind: &str, controls: Arc<Mutex<Controls>>, view: Option<Arc<Mutex<HostView>>>, static_dir: Option<String>) -> std::io::Result<()> {
  ...
  // `run_fixed`, never `run`. `run` delivers the measured elapsed time, which
  // would make the simulation's rate a property of the host's scheduler. In the
  // lattice examples that costs a correction; here there are no corrections, so
  // it would cost every client its agreement with the server, permanently.
