...
( cd "$root" && cargo build -p seed_defense --target wasm32-unknown-unknown --release --no-default-features --features web )

# 3. Place the artifact next to index.html. It is gitignored: a build product.
cp "$CARGO_TARGET_DIR/wasm32-unknown-unknown/release/seed_defense.wasm" "$here/static/seed_defense.wasm"
...
python3 "$root/../ws_client/check_js_imports.py" "$here/static/seed_defense.wasm" "$root/../ws_client/js/plaza_ws.js"

# 4. Shrink it if binaryen is available (optional; skipped silently otherwise).
if command -v wasm-opt >/dev/null 2>&1; then
  echo "==> optimising with wasm-opt"
  wasm-opt -Oz "$here/static/seed_defense.wasm" -o "$here/static/seed_defense.wasm"
fi
