...
( cd "$root" && cargo build -p curtain_fire --target wasm32-unknown-unknown --release --no-default-features --features web )

# 3. Place the artifact next to index.html. It is gitignored: a build product.
cp "$CARGO_TARGET_DIR/wasm32-unknown-unknown/release/curtain_fire.wasm" "$here/static/curtain_fire.wasm"
...
python3 "$root/../ws_client/check_js_imports.py" "$here/static/curtain_fire.wasm" "$root/../ws_client/js/plaza_ws.js"

# 4. Shrink it if binaryen is available (optional; skipped silently otherwise).
if command -v wasm-opt >/dev/null 2>&1; then
  echo "==> optimising with wasm-opt"
  wasm-opt -Oz "$here/static/curtain_fire.wasm" -o "$here/static/curtain_fire.wasm"
fi
