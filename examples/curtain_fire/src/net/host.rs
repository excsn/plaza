...
//! from the same port.
//!
//! The HTTP side of that is [`plaza_session::host::Host`]. What is left here is
//! the part that is actually this arena's: which state, which logic, at what
//! tick rate, and where the admission measurement comes from.
...
use plaza_session::host::{Host, init_logging};
...
pub async fn serve(bind: &str, controls: Arc<Mutex<Controls>>, view: Option<Arc<Mutex<HostView>>>, static_dir: Option<String>) -> std::io::Result<()> {
  ...
  // clients synchronise against, and what every input tick is named from.
  ...
  // One way, from the round trip the *server* measured. A client's word about
  // its own latency is the one number worth lying about, and this one decides
  // who gets in.
  let rtt = {
    let session = session.clone();
    Arc::new(move |key: &PlayerKey| session.agent_rtt(key).map(|(rtt, _)| rtt.as_millis() as u64 / 2)) as crate::net::arena::RttSource
  };

  let logic = ArenaLogic::new(controls, view).with_link(link).with_rtt(rtt).with_clock(sim_clock);
  let (commands, controller) = StateControllerBuilder::new(Arc::new(logic), session.clone(), Arc::new(NoSnapshots), Arena::new(initial, ARENA_SEED))
    // No snapshot on join: `Op::Welcome` carries the players and the policy
  ...
  // simulation's rate would be a property of the host's scheduler. Here that is
  // fatal rather than merely wrong: the curtain is a function of the tick, so a
  // tick worth a different amount of time on the server than on a client means
  // two different curtains, and nothing about a field neither end describes
  // would ever reveal the disagreement.
