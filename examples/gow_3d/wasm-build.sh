...
#
#    GOW_3D_FEATURES adds to that set, if you add any features worth passing.
features="web${GOW_3D_FEATURES:+,$GOW_3D_FEATURES}"
echo "==> building browser client (release wasm, features: $features)"
( cd "$root" && cargo build -p gow_3d --bin gow_3d --target wasm32-unknown-unknown --release --no-default-features --features "$features" )

# 3. Place the artifact next to index.html. It is gitignored: a build product.
cp "$CARGO_TARGET_DIR/wasm32-unknown-unknown/release/gow_3d.wasm" "$here/static/gow_3d.wasm"
...
python3 "$root/../ws_client/check_js_imports.py" "$here/static/gow_3d.wasm" "$root/../ws_client/js/plaza_ws.js"

# 4. Shrink it if binaryen is available (optional; skipped silently otherwise).
if command -v wasm-opt >/dev/null 2>&1; then
  echo "==> optimising with wasm-opt"
  wasm-opt -Oz "$here/static/gow_3d.wasm" -o "$here/static/gow_3d.wasm"
fi
