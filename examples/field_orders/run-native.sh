#!/usr/bin/env bash
# The zero-ceremony path: open the battlefield as a native desktop window. With
...
exec cargo run -p plaza_example_field_orders --bin field_orders --release --manifest-path "$root/Cargo.toml" -- "$@"
