...
use crate::sim::types::{Controls, MAZE_SEED, SIM_STEP_MS};
...
const WASM_FILE: &str = "pellet_maze.wasm";
...
pub async fn serve(bind: &str, controls: Arc<Mutex<Controls>>, view: Option<Arc<Mutex<HostView>>>, static_dir: Option<String>) -> std::io::Result<()> {
  ...
  let (commands, controller) = StateControllerBuilder::new(Arc::new(logic), session.clone(), Arc::new(NoSnapshots), Arena::new(initial, MAZE_SEED))
