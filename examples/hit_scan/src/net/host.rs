...
/// The arena's fixed layout seed. The map is a constant, so this only feeds the
/// bots' choices.
...
pub async fn serve(bind: &str, controls: Arc<Mutex<Controls>>, view: Option<Arc<Mutex<HostView>>>, static_dir: Option<String>) -> std::io::Result<()> {
  ...
  // could reproduce it. Here that would land as a rewind reaching a tick that
  // covered a different amount of time on each machine.
