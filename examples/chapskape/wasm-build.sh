...
#    the default set pulls in the native socket (tungstenite) and the actix
#    server, and neither compiles to wasm. `web` is the browser client alone.
#
#    CHAPSKAPE_FEATURES adds to that set, if you add any features worth passing.
features="web${CHAPSKAPE_FEATURES:+,$CHAPSKAPE_FEATURES}"
echo "==> building browser client (release wasm, features: $features)"
( cd "$root" && cargo build -p chapskape --bin chapskape --target wasm32-unknown-unknown --release --no-default-features --features "$features" )

# 3. Place the artifact next to index.html. It is gitignored: a build product.
cp "$CARGO_TARGET_DIR/wasm32-unknown-unknown/release/chapskape.wasm" "$here/static/chapskape.wasm"
...
python3 "$root/../ws_client/check_js_imports.py" "$here/static/chapskape.wasm" "$root/../ws_client/js/plaza_ws.js"

# 4. Shrink it if binaryen is available (optional; skipped silently otherwise).
if command -v wasm-opt >/dev/null 2>&1; then
  echo "==> optimising with wasm-opt"
  wasm-opt -Oz "$here/static/chapskape.wasm" -o "$here/static/chapskape.wasm"
fi
