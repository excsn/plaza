...
python3 "$root/../ws_client/check_js_imports.py" "$here/static/puck_rink.wasm" "$root/../ws_client/js/plaza_ws.js"

# 4. Shrink it if binaryen is available (optional; skipped silently otherwise).
if command -v wasm-opt >/dev/null 2>&1; then
  echo "==> optimising with wasm-opt"
  wasm-opt -Oz "$here/static/puck_rink.wasm" -o "$here/static/puck_rink.wasm"
fi
