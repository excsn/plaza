...
( cd "$root" && cargo build -p horde_playground --target wasm32-unknown-unknown --release )

# 3. Place the artifact next to index.html.
cp "$root/target/wasm32-unknown-unknown/release/horde_playground.wasm" "$here/static/horde_playground.wasm"
