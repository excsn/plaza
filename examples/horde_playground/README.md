...
|---|---|---|---|
| near | full precision position, health, shield | the player rate (10 Hz) | the main view, where a peer is drawn at full size |
| far | position quantised to one byte per axis | every 16th frame (about 0.6 Hz) | the minimap, where a peer is two pixels |
...
### The model was not the wire, until it was

Everything above is the server's **model** of what a packet costs. What actually crossed was `Handle { idx: u32, generation: u16 }` as a two-element array and `Vec2` as two 5-byte floats, so the panel's "the wire format costs Nx" line read about 3x. Two changes closed most of that gap, and neither touched the simulation:

- **The wire types carry what the model prices.** `Handle` serializes as one packed integer (`(idx << 16) | generation`, the same packing `SlotKey::encode` already uses for the digest, so there is not a second one to disagree with), and `Vec2` as two fixed-point `i32`s at 1/16 of a unit, which is two orders of magnitude under the smallest enemy radius and invisible everywhere a serialized position lands. Both are `#[serde(into/from)]` conversions, so the sim stays `f32` throughout. Sample 18 → 14 bytes, spawn 24 → 15, departure 17 → 7.
- **Fieldless enums cross as `u8`.** MessagePack spells a unit variant as its *name*, so every spawn was carrying six bytes of `"Swarm"` and every departure eleven of `"OutOfRange"`. `#[serde(into = "u8", try_from = "u8")]` with the numbers pinned in the conversions fixes it, and pinning them is the point: reordering the variants then cannot silently renumber the wire.

**The bigger saving was not encoding at all.** Every visible enemy was resampled in **every** packet, and under `Simulate` a sample is a *correction* to a rule the client is already running, not the motion itself. `sample_hz` (default 4) spreads the visible set round-robin across the packets, so each enemy is corrected four times a second while packets still leave at `sync_hz`. Spawns, departures and target changes are never deferred, and a target change bypasses the rotation outright because the server clears its announced-target record after each round, so a deferred one would never arrive rather than merely arriving late. Together with the encoding work this took the default arena from about 70 KiB/s to the low teens.

The lesson is the one this file keeps finding: **the byte cost was in the thing sent most often, not in the thing that looked expensive.** Quantising the whole wire was worth about 1.5x; not sending three quarters of the corrections was worth 4x.
...
The exemption list that used to be a match arm turned out to be unnecessary rather than relocated, because the loss model it compensated for was wrong. A WebSocket is TCP, and TCP retransmits: a lost segment never reaches the application as a missing message, it costs a retransmission timeout and stalls everything queued behind it. So the slider now buys a latency spike and a burst, which is what loss on this transport actually feels like, and nothing is deleted. The old `droppable` list was hand-modelling UDP over a reliable socket, which is precisely why it needed exceptions: `Welcome` and the other one-shots had no recovery, so they had to be excused from a hazard that could not have happened to them anyway.

Deleting frames is still available, as `Delivery::Datagram`, and it is the honest model for the unreliable channel plaza has yet to grow. Choosing it over a WebSocket simulates that transport deliberately, which is how this example's ack-and-digest recovery would get exercised before the channel it is for exists.
...
Two different answers to that hole live in this example, and the difference is worth noticing. For **enemies** it is aggregation, below: thousands of them, individually anonymous, so a summary per crowd is the right shape. For **players** it is [a coarse second tier](#relevance-has-two-axes-and-a-radius-is-only-one-of-them): a handful of them, individually named, so each keeps its own marker at map resolution. Same hole, and the shape of the answer follows from whether the things being summarised are interchangeable.
...
**The networked layer wraps that sim without touching it** ([src/net/](src/net/)). The server side is `plaza` core (`StateController`, `StateLogic`, `TickDriver`) over `plaza_session`; the arena seats joiners, buffers each seat's movement, and routes the entity-stream acknowledgement and the one purchase request straight to the authoritative server, so the whole loss-recovery and currency machinery now runs over a real socket. The client side is a `plaza_ws::Socket` plus clock/RTT estimation, an `InputCoalescer` deciding when to transmit, and nothing predicting the local player: it is drawn from the played-out stream at the same instant as everything else, which is why there is no reconciliation on it to measure. Cargo features name what you want to build: `client`, `server` (not on `web`), `native`, `web`, `websocket`.
