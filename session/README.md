...
For a delta-streaming simulation, `SimHost` is the whole stack behind that `Host`: the session with your protocol version and a simulation clock for its pongs, the controller with join snapshots off, a fixed-step driver, and a `/ws` route that numbers its connections. You hand it a codec, a protocol, a state and a closure that builds your `StateLogic` from the wiring (session handle, clock slot, a `link_sink()` for impairment panels). It is a prescription built from the blocks around it, so anything it decides for you (no join snapshot, numbered `u64` agents, `run_fixed`) is unmade by using those blocks directly. The one decision with a named alternative is the driver: `SimHost::measured(bind, tick_hz)` delivers measured elapsed time instead of fixed steps, for logic that only integrates over it and lets clients absorb the difference as corrections.
